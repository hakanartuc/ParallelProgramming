            ...
            for (int i = 0; i < 10; i++)
            {
                tasks.Add(Task.Factory.StartNew(()=> 
                {
                    for (int j = 0; j < 1000; j++)
                    {
                        ...
                        {
                            if (haveLook) mutex.ReleaseMutex();
                        }
                    }
                ...
                tasks.Add(Task.Factory.StartNew(() =>
                {
                    for (int j = 0; j < 1000; j++)
                    {
                        bool haveLook = mutex2.WaitOne();
                        try
                        {
                            ba2.Deposit(1); // deposit 10000 overall
                        }
                        finally
                        {
                            if (haveLook) mutex2.ReleaseMutex();
                        }
                    }
